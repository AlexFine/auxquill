...
def Summarize(title, text, nsent=5):
    summaries = []
    sentences = split_sentences(text)
    keys = keywords(text)
    titleWords = split_words(title)

    if len(sentences) <= nsent:
        return sentences

    #score setences, and use the top nsent sentences
    ranks = score(sentences, titleWords, keys).most_common(nsent)
    for rank in ranks:
...
def split_words(text):
    ...
    try:
        text = regex_sub(r'[^\w ]', '', text, REGEX_UNICODE)  # strip special chars
...
def keywords(text):
    ...
    keywords = dict((x, y) for x, y in freq.most_common(minSize))  # recreate a dict
...
def split_sentences(text):
    ...
    sentences = regex_split(u'(?<![A-ZА-ЯЁ])([.!?]"?)(?=\s+\"?[A-ZА-ЯЁ])', text, REGEX_UNICODE)
