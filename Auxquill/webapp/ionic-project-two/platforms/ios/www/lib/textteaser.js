...
JsTeaser.prototype.splitSentences = function () {
    ...
    var sentences = [];
    this.text.replace(/(.*?[.?!])(\s+|$)/g, function (all, sentence) {
...
JsTeaser.prototype.summarize = function () {
    ...
    var stopWords = this.stopWords;
//    alert("called n shit 304:"+sentences.join() +":"+titleWords.join() +":"+keys.join() +":"+stopWords.join());
    var ranks = score(sentences, titleWords, keys, stopWords);
//    alert("called n shit 305");
    var newRanks = ranks.map(function (element) {
        return element.key;
    });
//    alert("called n shit 308");
